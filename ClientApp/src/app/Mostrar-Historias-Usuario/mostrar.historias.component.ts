...
export class MostrarHistoriasComponent {
  ...
  constructor(http: HttpClient, @Inject("BASE_URL") baseUrl: string, rout: Router) {
    this.router = rout;
    http.get<Historias[]>(baseUrl + "api/Admin/obtenerHistoriasUsuario/" + parseInt(localStorage.getItem("idUsuario"))).subscribe(result => {
