...
lstUsuarios: Solicitudes[];
  ...
  actualizarProyecto() {
    ...
    this.http.put(this.baseUrl + "api/Admin/actualizarSolicitud/"+this.idTicket,this.idTicket).subscribe(res => {
      ...
      , () => {
       var grupo = parseInt((document.getElementById("grupo") as HTMLInputElement).value);
        console.log(this.idTicket);
        this.http.put(this.baseUrl + "api/Admin/AsignarProyectos/" + grupo + "/" + this.idTicket, this.idTicket).subscribe(res => {
        }, error => console.log(error)
          , () => {
